 ...
 *
 * SharedPreferences ile cache eklenmiştir. Tek bir HandlerThread kullanır.
 */
class AuxProbe(private val context: Context) {
    ...
    suspend fun probe(): List<Result> = withContext(Dispatchers.IO) {
        val cached = prefs.getString(KEY_OPENABLE_IDS, null)
        if (cached != null) {
            Log.i(TAG, "AuxProbe: loaded openable lenses from cache: $cached")
        ...
        // Cache the successful IDs
        prefs.edit().putString(KEY_OPENABLE_IDS, openableIds.joinToString(",")).apply()
